...
import static es.rgicr76.messaging.Queues.*;
...
@Component
public class MessageHandlerResta implements MessageHandler {

	private static final Logger LOG = LoggerFactory.getLogger(MessageHandlerResta.class);
	...
	@RabbitListener(queues = QUEUE_RESTA_BACKEND)
	public void handleMessage(@Payload EventoCalcular event) throws JsonProcessingException {
		...
		event.setOperator(calculadoraBackendService.resolverNodo(event.getOperator(), Operation.SUB,
				(value1, value2) -> value1 - value2));

		String siguienteOperacion = calculadoraBackendService.obtenerOperacion(event.getOperator(), Operation.SUB);
