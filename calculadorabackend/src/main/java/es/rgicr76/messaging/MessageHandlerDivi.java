...
@Component
public class MessageHandlerDivi implements MessageHandler {

	private static final Logger LOG = LoggerFactory.getLogger(MessageHandlerDivi.class);
	...
	@RabbitListener(queues = QUEUE_DIVID_BACKEND)
	public void handleMessage(@Payload EventoCalcular event) throws JsonProcessingException {
		...
		event.setOperator(calculadoraBackendService.resolverNodo(event.getOperator(), Operation.DIV,
				(value1, value2) -> value1 / value2));

		String siguienteOperacion = calculadoraBackendService.obtenerOperacion(event.getOperator(), Operation.DIV);
